...
vector<unsigned char> SHA256::compute(const string& input) {
  ...
  for (size_t chunk = 0; chunk < data.size(); chunk += 64) {
    ...
    for (int i = 0; i < 16; i++) {
      ...
      w[i] |= data[chunk + i*4 + 1] << 16;
      w[i] |= data[chunk + i*4 + 2] << 8;
      w[i] |= data[chunk + i*4 + 3];
    }
    for (int i = 16; i < 64; i++) {
...
string SHA256::toHex(const vector<unsigned char>& data) {
  stringstream ss;
  ss << hex << setfill('0');
  for (auto c : data) ss << setw(2) << (int)c;
  return ss.str();
}
